...
/**
 * The interactive half of the design reference. See `page.tsx` for why it is
 * split: KaTeX renders on the server, and a client boundary here would drag
 * 74 kB of it into the browser.
 */
  ...
  return (
    <TooltipProvider>
      <div className="space-y-12">
        ...
        {/* ------------------------------------------------------------------
            Phase 113 -- charts, table and the recording viewer.

            The charts bound to results render their absence, because the
            results directories are excluded from this export while the
        ...
        <section className="space-y-4">
          <SectionHeader
            ...
            title="Charts"
            description={
              'Every chart takes a generated table or figure and nothing else — there is ' +
              'no parameter that would accept a loose number. Marks are positioned from ' +
              'the numeric values array; all text comes from the display strings Python ' +
              'formatted. Fills whose colour fails contrast against the page are stroked, ' +
              'reading the same needs_outline_on measurement the figures use.'
            }
          />
          ...
          <GlassCard className="p-5">
            <GroupedBars
              ...
              caption="G01 — recording counts by dataset, from the audit."
            />
            <FigureDownload figureId="G01" className="mt-3" />
          </GlassCard>
          ...
          <GlassCard className="p-5">
            <ScatterPlot
              ...
              xName="Duration (s)"
              yName="Recordings"
              label="Recording duration against count"
              caption="G04 — the duration histogram drawn as points."
            />
            <FigureDownload figureId="G04" className="mt-3" />
          </GlassCard>

          <div className="grid gap-4 lg:grid-cols-2">
            <GlassCard className="p-5">
              <RocCurve xColumn="fpr" yColumn="tpr" label="ROC curve" />
            </GlassCard>
            <GlassCard className="p-5">
              <PrCurve xColumn="recall" yColumn="precision" label="Precision-recall curve" />
            </GlassCard>
            <GlassCard className="p-5">
              <ConfusionMatrix
                trueColumn="actual"
        ...
        <section className="space-y-4">
          <SectionHeader
            ...
            title="Results table"
            description={
              'Sortable, filterable, downloadable. Cells render the display strings only; ' +
              'the numeric array is used to sort and never reaches the DOM. The CSV is ' +
              'the text on screen, not a re-serialisation that could differ in the last ' +
              'decimal from the table it came from.'
            }
          />
          <ResultsTable table={tables['T02']} initialSort="n_records" />
        </section>
        ...
        <section className="space-y-4">
          <SectionHeader
            ...
            description={
              'Waveform, spectrogram and the cardiac-cycle overlay. Every band is one row ' +
              'of the dataset’s own segmentation file — nothing here is estimated or ' +
              'animated. It is a de-identified dataset sample, not a patient.'
            }
          />
          <RecordingViewer />
        </section>
